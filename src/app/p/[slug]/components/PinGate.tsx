...
export function PinGate({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      {brandLogoUrl && (
        <img src={brandLogoUrl} alt={agencyName} className="w-16 h-16 rounded-2xl object-cover mb-8 shadow-sm" />
      )}
      <div className="max-w-md w-full bg-white rounded-3xl shadow-sm border border-slate-200 p-8 text-center animate-in fade-in zoom-in-95 duration-500">
        <div className="w-16 h-16 bg-slate-100 text-brand rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex justify-center gap-3">
            {pin.map((digit, i) => (
              <input
                ...
                className={`w-14 h-16 text-center text-2xl font-bold rounded-xl border focus:outline-none focus:ring-2 focus:ring-brand transition-colors ${
          ...
          <Button 
            ...
            className="w-full h-12 bg-brand hover:opacity-90 text-white font-bold rounded-xl text-base shadow-sm transition-all"
          >
