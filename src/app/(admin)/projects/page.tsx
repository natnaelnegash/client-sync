...
import { Plus, Search, List, Grid, ChevronLeft, ChevronRight } from "lucide-react";
...
function getStatusDetails(status: string) {
  switch (status) {
    case "AWAITING_SIGNATURE":
      return { color: "bg-amber-100 text-amber-700", progress: 10, label: "Awaiting Signature" };
    case "COLLECTING_ASSETS":
      return { color: "bg-blue-100 text-blue-700", progress: 30, label: "Collecting Assets" };
    case "IN_PROGRESS":
      return { color: "bg-indigo-100 text-indigo-700", progress: 60, label: "In Progress" };
    case "IN_REVIEW":
      return { color: "bg-purple-100 text-purple-700", progress: 80, label: "In Review" };
    case "DELIVERY":
      return { color: "bg-pink-100 text-pink-700", progress: 90, label: "Delivery" };
    case "COMPLETED":
      return { color: "bg-emerald-100 text-emerald-700", progress: 100, label: "Completed" };
    default:
      return { color: "bg-slate-100 text-slate-700", progress: 0, label: "Draft" };
  }
}
...
export default async function ProjectsPage() {
  ...
  const activeCount = projectsData.filter(p => p.status !== 'COMPLETED').length;
      ...
      <div className="flex items-center justify-between gap-4 py-2">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-2 sm:pb-0 w-full sm:w-auto scrollbar-hide">
            <button className="px-4 py-1.5 rounded-full bg-indigo-600 text-white text-sm font-medium whitespace-nowrap shadow-sm">
              All
            </button>
            <button className="px-4 py-1.5 rounded-full bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium whitespace-nowrap shadow-sm">
              Awaiting Signature
            </button>
            <button className="px-4 py-1.5 rounded-full bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium whitespace-nowrap shadow-sm">
              In Progress
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {projectsData.map((project) => {
                const { color, progress, label } = getStatusDetails(project.status);
                const valueFormatted = project.totalValue 
                  ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(Number(project.totalValue) / 100)
                  : '$0';
                ...
                return (
                  <tr key={project.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-5">
                      <Link href={`/projects/${project.slug}`} className="font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors">
                        {project.name}
                      </Link>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm text-slate-500">{project.clientOrg || '—'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5">
                      <span className="font-medium text-slate-900">{valueFormatted}</span>
                    </td>
        ...
        {projectsData.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-200 bg-white flex items-center justify-between">
            <p className="text-sm text-slate-500">
              Showing 1–{projectsData.length} of {projectsData.length}
            </p>
            <div className="flex items-center gap-1">
              <button className="w-8 h-8 flex items-center justify-center rounded border border-slate-200 text-slate-400 hover:text-slate-600 hover:bg-slate-50" disabled>
